...
- `Dockerfile`: multi-stage native build from the post, plus one extra line: the native binary needs `libz.so.1`, which `distroless/base` does not ship, so the container fails at startup (`error while loading shared libraries: libz.so.1`) without copying it from the builder stage.
...
```
The folder includes the Maven wrapper (`mvnw`) that the `Dockerfile` uses: `docker build -t native-demo .`, then `docker run -p 8080:8080 native-demo`.
...
Status: `mvn verify` passes (hints test) on JDK 25, Maven 3.9.9, 2026-09-19. `mvn -Pnative spring-boot:build-image` (buildpacks, BellSoft Liberica NIK 25) built a 236 MB image in 9m37s, and `GET /greet/Ada` returned the expected text. The `Dockerfile` build (about 5-6 min, 140 MB image) also works, and that container started in 0.21 s and answered `/greet/Ada`.
